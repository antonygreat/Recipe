class IngredientSetsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @ingredient_set.save
        ...
        format.json { render action: 'show', status: :created, location: @ingredient_set }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to ingredient_sets_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def ingredient_set_params
      params.require(:ingredient_set).permit(:recipeID, :ingredientID, :amountNum, :unitOfMeasure)
    end
end
